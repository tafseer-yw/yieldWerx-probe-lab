...
export const assessmentCatalogue: Assessment[] = [
  ...
  },
  {
    ...
    track: 'qa',
    order: 12,
    title: 'Design the performance run',
    effort: 'advanced',
    skills: ['/yw:forge-performance-tests'],
    mission:
      'Design — not yet run — the performance test for the wafer list under a few hundred wafers: workload shape, the operation under load, and objectives traced to an approved number. If no number exists, propose one and get it approved; an invented target tests an opinion.',
    passWhen: [
      'Workload profile chosen and justified in a sentence each',
      'Every objective traces to an approved number, none invented',
      'The design names what would make the run invalid',
    ],
  },
  {
    id: 'qa-13',
    ...
    mission:
      'Write and hand-run the access-control cases for this app: a viewer must not upload or delete, a qa user must not manage sample wafers, an expired token must fail closed. Scanners cannot judge who should be allowed to do what — that takes a person.',
    passWhen: [
      'Cases cover each role boundary the app claims to enforce',
      'Each case tried by hand with the verdict recorded',
      'Tagged with their OWASP category',
      'Any hole found is filed as a bug, severity justified',
    ],
  },
  {
    id: 'qa-14',
  ...
  },
  {
    ...
    mission:
      'Take a feature someone else built — a dev capstone, or the bin pareto CSV export — through the whole QA track: spec analysis, cases, a recorded gate approval, recon, automation, a mutation audit, a three-run streak, and the merge-gate digest. Every stage leaves its artifact.',
    passWhen: [
      ...
      'Automation passed the mutation audit',
      'The merge-gate digest is complete, gaps included',
    ],
  },
];
