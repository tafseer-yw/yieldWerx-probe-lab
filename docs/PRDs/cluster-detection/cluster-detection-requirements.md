...
| **CLD-08** | **Adjacency.** Two candidates are connected when their coordinates differ by one step, where a **step is the wafer's die pitch** — the greatest common divisor of the gaps between the distinct coordinates present, per axis. It is 1 for a CSV written in die indices and commonly 5 for an ATDF written in stepper positions. `4-way` counts the four edge neighbours (one step in `x`, or one step in `y`). `8-way` also counts the four corner neighbours. Adjacency is therefore invariant under a uniform coordinate scaling: the same physical wafer recorded at a wider pitch yields the same clusters. |
...
| **CLD-11** | **Gaps break adjacency.** A grid position carrying no die is not a candidate and cannot join two components. Two failing dies either side of a missing position are separate clusters — a missing position being one whole die pitch, not one raw coordinate unit. There is no wrap-around at the edge of the data.                                                                                                                                                                                                                                                                                              |
...
- The die pitch derivation, and detection proven invariant under a uniform coordinate
  scaling, so a wide-pitch wafer is not silently reported as cluster-free.
